...
// signNonceHeader carries the client-supplied challenge bound into the signed
// authz envelope. It mirrors the header name enforced by the client signature
// middleware in the handlers package.
const signNonceHeader = "X-Nonce"
...
// BuildMaintenanceInfo renders the maintenance banner for an app, or nil when
// maintenance is disabled/unsupported.
func (h *Handler) BuildMaintenanceInfo(app models.App) *maintenanceInfo {
	if !schema.HasAppMaintenanceColumn(h.DB) || !app.MaintenanceEnabled {
		return nil
	}
...
func (h *Handler) UpdateCheck(c *gin.Context) {
	...
	if err := h.DB.Where(
		...
		if link := strings.TrimSpace(matched.ExternalDownloadURL); link != "" && schema.HasReleaseExternalDownloadURLColumn(h.DB) {
