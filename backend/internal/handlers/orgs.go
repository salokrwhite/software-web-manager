...
func (h *Handler) UpgradeOrg(c *gin.Context) {
	...
	materials, statusCode, err := h.storeAttachments(c, attachmentOwnerOrgRegistrationMaterial, orgUUID, &orgUUID, nil, "materials", filepath.ToSlash(filepath.Join("orgs", orgUUID.String(), "registration_materials")), len(files), maxEnterpriseMaterialSize)
	if err != nil {
		if statusCode == 0 {
