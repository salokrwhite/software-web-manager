...
func renderRegisterEmailCodeTemplate(content, siteName, code string, expiresMinutes int) string {
	...
	if template == "" {
		template = defaultRegisterEmailCodeTemplate
	}
...
func (h *Handler) getRegisterEmailContext() (smtpConfig, string, string, error) {
	if !h.hasSystemSettingsTable() {
		return smtpConfig{}, "", "", errRegisterEmailNotConfigured
	}
	...
	if passwordErr != nil {
		return smtpConfig{}, "", "", passwordErr
	}
	cfg.Password = password
	if err := validateSMTPConfig(cfg, !configured); err != nil {
		return smtpConfig{}, "", "", err
	}
	siteName := getStringSetting(items, systemSettingSiteNameKey, defaultSiteName)
	template := getStringSetting(items, systemSettingRegisterEmailCodeTemplateKey, defaultRegisterEmailCodeTemplate)
...
func (h *Handler) SendRegisterEmailCode(c *gin.Context) {
	...
	if subjectSiteName == "" {
		subjectSiteName = defaultSiteName
	}
	...
	if err := sendSMTPMail(cfg, email, subject, body); err != nil {
