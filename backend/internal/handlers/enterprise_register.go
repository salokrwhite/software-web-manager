...
func (h *Handler) EnterpriseRegister(c *gin.Context) {
	...
	for _, file := range files {
		if file.Size > maxEnterpriseMaterialSize {
	...
	orgID := uuid.New()
	materials, statusCode, err := h.storeAttachments(c, attachmentOwnerOrgRegistrationMaterial, orgID, &orgID, nil, "materials", filepath.ToSlash(filepath.Join("orgs", orgID.String(), "registration_materials")), len(files), maxEnterpriseMaterialSize)
	if err != nil {
		if statusCode == 0 {
...
func (h *Handler) EnterpriseResubmit(c *gin.Context) {
	...
	materials, statusCode, err := h.storeAttachments(c, attachmentOwnerOrgRegistrationMaterial, org.ID, &org.ID, nil, "materials", filepath.ToSlash(filepath.Join("orgs", org.ID.String(), "registration_materials")), len(files), maxEnterpriseMaterialSize)
	if err != nil {
		if statusCode == 0 {
			statusCode = http.StatusInternalServerError
		}
