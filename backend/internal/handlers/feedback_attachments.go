...
func (h *Handler) storeFeedbackAttachments(c *gin.Context, feedbackID uuid.UUID, orgID uuid.UUID) ([]models.Attachment, int, error) {
	return h.storeAttachments(c, attachmentOwnerFeedback, feedbackID, &orgID, nil, "attachments", "feedbacks", maxFeedbackAttachments, maxFeedbackAttachmentSize)
}

func (h *Handler) loadFeedbackAttachments(c *gin.Context, feedbackID string) ([]feedbackAttachmentResponse, error) {
	items, err := h.loadAttachmentResponses(c, attachmentOwnerFeedback, feedbackID)
	if err != nil {
		return nil, err
	}
