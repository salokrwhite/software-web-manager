...
func (h *Handler) CreateOrgInvite(c *gin.Context) {
	...
	invite := models.OrgInvite{
		...
		Email:     req.Email,
		Role:      req.Role,
		TokenHash: utils.SHA256Hex([]byte(rawToken)),
...
func (h *Handler) AcceptOrgInvite(c *gin.Context) {
	...
	if err := h.DB.Where("token_hash = ?", utils.SHA256Hex([]byte(token))).First(&invite).Error; err != nil {
	...
	} else if req.Password != "" && !utils.CheckPassword(user.PasswordHash, req.Password) {
