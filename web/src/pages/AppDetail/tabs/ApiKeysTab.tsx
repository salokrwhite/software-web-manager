...
export default function ApiKeysTab({
  ...
  const createAppSecret = async () => {
    ...
    try {
      ...
      const res = await api.post(`/api/apps/${appId}/api-keys`, payload)
  ...
  const revokeAppSecret = async (targetKeyId: string) => {
    ...
    try {
      await api.delete(`/api/api-keys/${targetKeyId}`)
      message.success('已撤销')
      reload()
    } catch (err: any) {
  ...
  const handleConfirmCreatedSecret = async () => {
    ...
    try {
      ...
      await api.patch(`/api/api-keys/${createdSecret.keyId}/policy`, {
