...
export default function SystemTicketDetail() {
  ...
  const loadDetail = async () => {
    ...
    setLoading(true)
    try {
      const res = await api.get(`/api/system/tickets/${id}`)
      setDetail(res.data)
    } catch (err: any) {
  ...
  const loadMessages = async () => {
    ...
    try {
      const res = await api.get(`/api/system/tickets/${id}/messages`)
  ...
  const updateStatus = async (status: 'in_progress' | 'resolved') => {
    ...
    try {
      await api.patch(`/api/system/tickets/${detail.id}/status`, { status })
  ...
  const handleDelete = () => {
    ...
    Modal.confirm({
      ...
      okType: 'danger',
      onOk: async () => {
        await api.delete(`/api/system/tickets/${detail.id}`)
  ...
  const sendMessage = async () => {
    ...
    setSending(true)
    try {
      await api.post(`/api/system/tickets/${id}/messages`, formData)
      setMessageContent('')
      setMessageFiles([])
      await loadMessages()
    } catch (err: any) {
  ...
  const canStart = detail?.status === 'submitted'
  const canResolve = detail?.status === 'in_progress'
  ...
  return (
    <div>
      <Space direction="vertical" size={4} style={{ marginBottom: 16 }}>
        ...
        <Text type="secondary">查看工单内容并更新处理状态</Text>
      </Space>
          ...
          <Card style={{ flex: 1, minWidth: 0, width: '100%', borderRadius: isMobile ? 10 : 12 }}>
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              ...
              <Space direction={isMobile ? 'vertical' : 'horizontal'} style={isMobile ? { width: '100%' } : undefined}>
                ...
                <Button danger onClick={handleDelete} style={isMobile ? { width: '100%' } : undefined}>
                  删除工单
                </Button>
              </Space>
            </Space>
