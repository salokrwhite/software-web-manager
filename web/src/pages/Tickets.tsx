import { Button, Card, Form, Grid, Modal, Select, Space, Table, Tag, Typography, message } from 'antd'
...
export default function Tickets() {
  ...
  const loadTickets = async (params?: any, page = current, size = pageSize) => {
    ...
    try {
      ...
      const res = await api.get('/api/tickets', { params: { ...params, limit, offset } })
  ...
  const handleBatchDelete = () => {
    ...
    Modal.confirm({
      ...
      okType: 'danger',
      onOk: async () => {
        await api.post('/api/tickets/batch-delete', { ids: selectedRowKeys })
  ...
  return (
    <div>
      <Space direction="vertical" size={4} style={{ marginBottom: 16 }}>
        ...
        <Text type="secondary">查看企业历史工单记录</Text>
      </Space>
      ...
      <Card style={{ marginBottom: 16, borderRadius: isMobile ? 10 : 12 }}>
        ...
        >
          <Form layout={isMobile ? 'vertical' : 'inline'} form={form} initialValues={{ status: 'all' }} style={isMobile ? { width: '100%' } : undefined}>
            <Form.Item name="status">
              <Select style={{ width: isMobile ? '100%' : 180 }}>
          ...
          columns={[
            ...
            },
            {
              ...
              render: (_: any, row: TicketItem) => (
                <Button size="small" onClick={() => navigate(`/tickets/${row.id}`)}>{isMobile ? '详情' : '查看'}</Button>
              )
            }
          ]}
        />
